...
defineOptions({
  name: "ChartCourseGlance"
});
const dataLoading = ref(true);
const practiceId = ref(props.id);
const table = ref([]);
  ...
  try {
    await pageInfo(param)
      .then(data => {
        ...
        if (data.code === 200) {
          table.value = data.data.records.map(decode).map(convertRecord);
          init();
          return;
        }
...
const convertRecord = record => {
  ...
  return {
    name: city,
    value: Number.parseInt(record.desc)
  };
};
...
<template>
  <el-container>
    <el-main>
      ...
      >
        <div ref="chartRef" style="width: 100%; height: 80vh" />
      </div>
    </el-main>
  </el-container>
</template>
